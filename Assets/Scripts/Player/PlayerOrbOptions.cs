 ...
 * It handles the player's ability to heal the zombies. The biggest responcibility
 * of this script is to maintain the timing of beam cooldowns so that the player 
 * cannot heal too fast. Mostly, this is a "pass through" or "bridge" script, 
 * which means that it receives input from the PlayerInputController script and t
 * then passes the input along to the appropriate beam. Very little beam logic 
 * (apart from timing) exists in this script.  
 * 
 * Author: Bruce Gustin
