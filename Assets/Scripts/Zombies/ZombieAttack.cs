...
public class ZombieAttack : MonoBehaviour
{
	...
	[HideInInspector] public HealingShroudDebuff healingShroudDebuff;		//Reference to a slime debuff that may prevent attacking
	...
	IEnumerator AttackPlayer()
	{
		...
		//This is usefull if you start with an zombie in the scene (instead of spawning it)
		yield return null;
		...
		//While the zombie can attack and the player isn't defeated...
		while (canAttack && CheckPlayerStatus())
		{
			//...and if the player is in range and the enemy isn't slimed...
			if (playerInRange && healingShroudDebuff == null) 
			{
	...
	//Called when the zombie needs to stop attacked (either it the player was defeated or the zombie is resting)
	public void Resting()
	{
		//Enemy can no longer attack
		canAttack = false;
		//If a slime debuff exists on the enemy, remove it
		if (healingShroudDebuff != null)
			healingShroudDebuff.ReleaseZombie();
	}
}
