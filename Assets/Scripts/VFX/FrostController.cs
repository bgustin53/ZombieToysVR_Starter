...
{
	[Header("Weapon Specs")]
	[SerializeField] int maxFreezableZombies = 20;      //How many enemies can be freezed at once. This is helpful as it limits the number of frost debuffs that need to be tracked and thus reduces lag
	...
	void Awake()
	{
		...
		debuffs = new FrostDebuff[maxFreezableZombies];
		//Loop through the array...
		for (int i = 0; i < maxFreezableZombies; i++)
		{
	...
	//Called from PlayerOrbOptions script and OnDisable()
	...
	void AttachDebuffToEnemy(ZombieMovement zombie)
	{
		...
		for (int i = 0; i < debuffs.Length; i++)
		{
			...
			if (!debuffs[i].gameObject.activeInHierarchy)
			{
				...
				debuffs[i].AttachToZombie(zombie);
	...
	void OnTriggerEnter(Collider other)
	{
		...
		if (zombie.FrostDebuff != null)
			zombie.FrostDebuff.AttachToZombie(zombie);
		//If there isn't already a frost debuff attached, attach a new one
		else
			AttachDebuffToEnemy(zombie);
	}
	...
	void OnTriggerExit(Collider other)
	{
		...
		if (zombie.FrostDebuff != null)
			zombie.FrostDebuff.ReleaseEnemy();
	}
}
