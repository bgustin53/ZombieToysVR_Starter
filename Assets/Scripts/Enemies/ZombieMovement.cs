 ...
 * This script controls the movement of the zombies. The zombies uses a navmesh agent to 
 * navigate the scene. The target of the zombie's navigation is received from the GameManager
 * and that allows the zombie to chase whatever the GameManager wants. Additionally, the zombie
 * movement can be affected by a frost debuff which freezes it in place
 * 
 * Author: Bruce Gustin
...
public class ZombieMovement : MonoBehaviour
{
	...
	[SerializeField] ZombieHealth zombieHealth;							//Reference to the zombie health component

	[Header("Stink Hit Properties")]
	[SerializeField] float runAwayDistance = 10f;						//How far the enemy runs when hit by a stink attack

	
	private Transform plushyHome;										//Where the cured zombie goes.
	private float originalSpeed;                                        //Original movement speed of the zombie (in case they get frozen)
	private bool isRunningAway;                                         //Is the zombie running away?
	private Vector3 runAwayPosition;                                    //Where the zombie runs if they are hit with a stink attack

	private static WaitForSeconds updateDelay = new WaitForSeconds(.5f); //The delay between updating the navmesh agent (for efficiency). Since
																		 //all zombie have the same delay, this is declared as 'static' so all
																		 //zombie share the same one (saves on memory)

	void Start()
	{
		// Get the prefab name with "(Clone)" removed then gets target name
	...
	IEnumerator ChasePlayer ()
	{
		...
		{
			//... If last zombie, get home otherwise get the target from the GameManager...
			Transform target = zombieHealth.headHome ? plushyHome : GameManager.Instance.ZombieTarget;
	...
	//Called when the zombie is resting and can no longer move
	public void Resting()
	{
	...
	//This method is called when the zombie is hit by a stink attack
	public void Runaway()
	{
		//The enemy is now running away
		isRunningAway = true;
		//Get a vector from the player's position to the enemy's position
		Vector3 runVector = transform.localPosition - GameManager.Instance.ZombieTarget.position;
